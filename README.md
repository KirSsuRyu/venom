...
```bash
npx @kirssu/venom-init --dry-run        # 변경 없이 계획만 출력
npx @kirssu/venom-init --from-git       # 동봉본 대신 GitHub main에서 직접 가져오기
npx @kirssu/venom-init --from-git --ref some-branch
npx @kirssu/venom-init --no-backup --force   # 백업 없이 덮어쓰기 (위험)
```
