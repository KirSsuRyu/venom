...
### Added
- **`00-core.md` 규칙 9 — 수술적 변경** (Karpathy Principle 3 흡수):
  *요청에서 직접 비롯되지 않은* 줄은 건드리지 않는다. 기존 dead code는
  보고만 하고 삭제는 요청받았을 때만. 단, **내 변경이 만든** 고아
  (import·변수·함수)는 반드시 내가 정리한다.
  검사 기준: "변경된 모든 줄이 사용자 요청으로 직접 추적되는가?"
- **`00-core.md` 규칙 10 — 가정을 드러낸다** (Karpathy Principle 1 흡수):
  모호한 요청에 조용히 해석해 진행하지 않는다. 핵심 가정을 명시적으로
  선언하고, 트레이드오프를 드러내며, 더 단순한 방법이 있으면 역제안한다.
- **`10-coding-standards.md` — 단순성 우선** (Karpathy Principle 2 흡수):
  구조 섹션 첫 항목으로 "단순성 우선" 추가. 투기적 추상화 금지.
  "시니어 개발자가 너무 복잡하다고 할 것 같으면 단순하게 다시 쓴다" 구체적
  검사 기준 명시.
- **`10-coding-standards.md` — 죽은 코드 구분 명확화**: 내 변경이 만든 고아는
  정리(의무), 기존 dead code는 보고만(선택). 기존 "제거한다" 단일 규칙의
  모호성 해소.
- **`CLAUDE.md` 의무 작업 루프 확장**: `이해 → 계획` 앞에 "가정 선언 + 성공
  기준 정의" 단계 명시. 각 단계 구체화 — 특히 착수 전에 "완료"가 무엇인지
  선언하고 검증 방법을 미리 결정하도록 강제.
- **`decisions.md` ADR-0003**: 흡수 결정과 근거 기록.

### Verified
- `npm test` 통과.
- 규칙 3개 사람 리뷰 — 기존 규칙과 중복 없음, 새 영역만 다룸.
- 참고: github.com/forrestchang/andrej-karpathy-skills
